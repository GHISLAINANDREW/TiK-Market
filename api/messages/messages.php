...
try {
    ...
    // Auto-migration (individual checks)
    $cols = [
    ...
    foreach ($cols as $col => $def) {
        try {
            $check = $db->query("SHOW COLUMNS FROM messages LIKE '$col'")->fetch();
            if (!$check) {
    ...
    if ($method === 'POST') {
        ...
        if ($audio_url) {
            ...
            if (strpos($base64, 'data:') === 0) {
                if (preg_match('/^data:([^;]+);base64,/', $base64, $matches)) $mimeType = strtolower($matches[1]);
            ...
            if ($fileData) {
                ...
                if ($uploadedUrl) $audio_url = $uploadedUrl;
            }
        }
